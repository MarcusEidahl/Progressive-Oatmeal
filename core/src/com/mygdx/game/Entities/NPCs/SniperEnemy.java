...
public class SniperEnemy extends NPC {
    ...
    public SniperEnemy(float x, float y, Level level, Entity target) {
        super(x, y, level);
        this.target = target;
    }
    ...
    private void shoot() {
        if(fireTimer >= timeBetweenShots) {
            level.addEntity(new com.mygdx.game.Entities.Projectiles.SniperProjectile(x, y - radius, level, new Vector2(x - target.x, y - target.y).nor(), this));
            fireTimer -= timeBetweenShots;
        }
    }
